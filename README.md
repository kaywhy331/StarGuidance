...
StarGuidance is a private tarot consultation in which a cryptographically committed shuffle and reader-selected hidden positions finalize the cards only after the visual ritual. The reader asks one question; the server recommends the smallest suitable active spread from that question before any card is known. Profile and question data never enter card selection. Visitors can complete one birthday-personalized deterministic reading without an account, then sign up or sign in to recover those exact cards and ask a same-scope follow-up. The guest lane sends the birthday only to the private calculation service, uses stable date-derived traits, discards the raw date before building the browser-held receipt, and makes no AI-provider request. The safe beta also supports email/password accounts, private onboarding and profile correction, immutable profile snapshots, an encrypted directory of permission-confirmed people for explicit `@mention` relationship context, six configurable spreads, high-stakes preflight, exact-draw recovery, spread-aware interpretation fallback, configurable same-draw follow-ups and reading allowances, feedback, history, and privacy controls. A 17-section profile report, purchased-report history, test Checkout lifecycle, durable fulfillment, and authenticated tagged PDF are implemented behind a default-off release gate; owner commerce, accessibility-review, and production-provider gates still block public launch.

The reading ritual uses the accepted responsive cosmic Gothic sanctuary, exactly 78 lightweight card backs in a casino-wash field, a gather-and-fan selection gesture, card-specific illustrated faces, a physical 3D card system, ordered accessible reveals, and an authenticated streaming oracle transcript. Hover, keyboard focus, click, or an upward swipe selects each anonymous card and deals it into the next spread position. Whole-reading prose stays hidden until every card is revealed. The spread then remains visible beside a compact window that presents one concise passage at a time; audio advances that same passage and highlights its referenced card. Personalized readings weave only eligible, question-relevant profile traits—and, when explicitly mentioned, a minimized relationship lens—into the direct answer and card interpretations while retaining a labeled audit record; unsupported trajectory, alternate-path, and timing sections remain absent. See [artwork provenance](docs/ARTWORK-PROVENANCE.md) for sources, prompts, rights boundaries, hashes, and performance budgets.
...
- `packages/tarot-domain` — committed two-party entropy, unbiased shuffle and hidden-position selection, optional reversals, and immutable draw rules.
- `packages/tarot-content` — original, versioned 78-card content and six recommendable spreads.
