...
The shuffle animation renders nine lightweight shells, not 78 complex cards. Cutting cannot modify the already locked assignment. The sanctuary offers an explicit deck-cut choice (Cut / Skip cut) and intentional per-card click/tap/keyboard reveal with a Reveal-all convenience control; none of it can change the already-persisted assignment, only how it is watched. `{cutTaken, revealedIndexes}` is validated and stored in per-reading session storage for exact interaction recovery. Completed history entries use the pre-revealed `/reading/[id]` result route. See [Known gaps](KNOWN-GAPS.md) for the remaining manual keyboard/screen-reader/real-device review.
