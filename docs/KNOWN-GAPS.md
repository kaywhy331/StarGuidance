...
- The current sanctuary automatically skips the cut and reveals cards in sequence. It has reduced-motion and sound controls but no immediate “Skip animation” control and no user-controlled click/tap/keyboard reveal. Those are explicit PRD experience requirements and must be implemented and accessibility-tested before calling the ritual complete.
...
- Current request limits are bounded and ignore spoofable `x-forwarded-for`, but they are in-process only. A distributed, privacy-safe limiter is required before multi-instance/serverless public traffic.
