...
Durable asynchronous report generation, retry/status handling, title-only preview, structured web report, browser print/PDF, provider-accurate labeling, and profile/payment separation are implemented. The credential-free local adapter deliberately remains synchronous and is not evidence of Stripe. Public launch still requires the credentialed procedure above, standalone purchased-report history/support diagnostics, an accessible generated PDF with web/PDF parity tests, pricing/tax/receipt policy, and approved finance retention. The browser-printable page alone must not be marketed as an accessible PDF artifact.
