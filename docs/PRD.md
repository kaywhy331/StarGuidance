...
| **UX-012** | The visual system is minimal, immersive, and content-led. | **Must** | Attention follows one explicit hierarchy: cards during shuffle/deal/reveal; a compact visible spread supporting one interpretation surface during narration; next actions only after deliberate reading completion; and all other utilities collapsed, hidden, or visually discreet. Card and reading stages use an original responsive clear starry-night backdrop with a dark central field, high contrast, and no distracting decorative motion during reading text. |
