...
9. Intentional card reveal, a full-screen shuffle that gathers directly into the deal, and an explicit Deal now animation shortcut are implemented and e2e-covered (updated 2026-08-15). Background interpretation and report generation use leased durable jobs with the same authenticated Netlify-scheduled backstop (migrations `0007` and `0012`); still open per [Known gaps](KNOWN-GAPS.md): manual keyboard/screen-reader/real-device review of the ritual controls.
