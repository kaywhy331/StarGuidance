...
Current draft PR #19 preview: [deploy-preview-19--starguidance.netlify.app](https://deploy-preview-19--starguidance.netlify.app). Verify `/api/health` reports the intended final commit after every trigger before accepting any hosted evidence. The credentialed workflow is restricted to `agent/starguidance-immersive-ux` and the protected `starguidance-staging` environment. No production deployment has been performed or implied.
...
8. Distributed rate limiting and privacy-safe aggregate measurement/alert delivery are implemented. Before public traffic, configure and rehearse the managed alert receiver plus provider-native host/database/billing alerts, and approve cross-service trace correlation. `main` now requires PRs, required checks, and conversation resolution (verified 2026-08-06); an independent approving reviewer is still unassigned.
