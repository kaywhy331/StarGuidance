...
| `FISH_AUDIO_API_KEY`                         | optional managed secret               | isolated UAT key only, server-only                       | rotated managed secret          | Bearer key for the fixed `https://api.fish.audio/v1/tts/stream/with-timestamp` endpoint  |
| `FISH_AUDIO_REFERENCE_ID`                    | `84aadb575ed74e9aa44ba7ab8619d1a1`    | same ID only after voice UAT                             | approved voice ID               | Non-secret fixed voice selection; never accepted from the browser                        |
...
12. Keep `READING_AUDIO_PROVIDER=disabled` until Fish Audio commercial-plan terms, privacy/retention settings, usage permission for reference `84aadb575ed74e9aa44ba7ab8619d1a1`, cost controls, device-cache disclosure, and point-of-use copy are approved. Then run credentialed tests proving no request on load or preference enablement, one request on Play, lazy next-section loading, timestamp/word/card-aura synchronization, pause/stop behavior, provider failure recovery, replay-without-regeneration across reload, shared-device cache clearing, and current Chrome/Firefox/Safari plus iOS/Android playback before promotion.
