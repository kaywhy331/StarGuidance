...
Release record, 2026-08-21: PR #19 merged as `55747fc2a4ff4af33120f91cea3e1f48bddc1d7b`. Netlify production deploy `6a889b71f468000008b8743b` published that exact commit and returned `200` from `/api/health`, `/`, `/sign-in`, `/sign-up`, `/terms`, and `/privacy`; the preview-only route returned `404`. Follow-up PR #21 merged the test-only commit `6ce96532e459ada36176969f59e92bc5c8959262`, whose final main CI passed all 52 jobs. Because the follow-up changed only three E2E specs, the production runtime remains deploy `6a889b71f468000008b8743b` with unchanged application source. The pre-release rollback anchor is deploy `6a839aeb9399100008c982db` at `3bf6ad56804fcd716a40fe9cf68cf0074e47fca0`.
...
8. Distributed rate limiting and privacy-safe aggregate measurement/alert delivery are implemented. Before public traffic, configure and rehearse the managed alert receiver plus provider-native host/database/billing alerts, and approve cross-service trace correlation. `main` requires PRs, required checks, and conversation resolution (verified 2026-08-06). An independent reviewer remains unassigned; the owner explicitly waived that review for PR #19 and the test-only corrective PR #21 on 2026-08-21. That one release decision is not a standing waiver for later merges or activation gates.
