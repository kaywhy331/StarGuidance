...
For a link that works when an email application opens a different browser, the Supabase **Magic Link** email template must send the one-time token hash directly to the reviewed callback. Use this exact link target; `RedirectTo` already contains the safe `next` query generated by the application:
