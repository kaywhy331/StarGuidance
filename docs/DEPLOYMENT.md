...
12. Keep `READING_AUDIO_PROVIDER=disabled` until Fish Audio commercial-plan terms, privacy/retention settings, the exact voice's usage permission, cost controls, device-cache disclosure, and point-of-use copy are approved. Then run credentialed tests proving no request on load or preference enablement, one request on Play, lazy next-section loading, timestamp/word/card-aura synchronization, pause/stop behavior, provider failure recovery, replay-without-regeneration across reload, shared-device cache clearing, and current Chrome/Firefox/Safari plus iOS/Android playback before promotion.
