...
- Browser JWTs have no private-table privileges. User-scoped SQL transactions assume the non-login, non-inheriting `starguidance_app` role and set only the subject obtained from verified Supabase Auth. RLS is forced on user-owned tables; anonymous/public access is revoked. The interpretation-job queue is subject-bound for the application role too (migration `0008`): a request-path transaction can reach only its own subject's job rows, while the cross-user claim/complete/fail sweep runs as the owning connection role under an explicit named policy — the same trust boundary as the payment-webhook lease.
- Mutating browser routes validate Origin/Host. Rate limits on the `supabase` runtime adapter are enforced by a distributed, Postgres-backed function (migration `0006_rate_limit_buckets`, forced RLS, `starguidance_app`-only); the `local` adapter (dev/test) keeps an in-process limiter. Stripe webhooks are exempt from Origin checks and require signature verification.
